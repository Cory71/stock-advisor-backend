...
//   5. Recent free cash flow growth (TTM)   — TTM FCF > most recent annual FCF AND TTM FCF > 0
//
// Criteria 4 and 5 require the most recent value to be positive: a company that
// merely shrank its loss (e.g. -$3B to -$1B) is still burning cash, so "less
// negative" must not count as healthy free-cash-flow growth.
//
...
// When `requirePositive` is true, the latest value must also be > 0 to pass —
// used for the FCF-growth checks so a still-negative (cash-burning) company
// that merely shrank its loss does not count as growth.
function buildCriterion(name, value, prior, source, requirePositive = false) {
  if (value == null || prior == null) {
    return { name, passed: null, value: value ?? null, prior: prior ?? null, source };
  }
  const passed = value > prior && (!requirePositive || value > 0);
  return { name, passed, value, prior, source };
}
...
function gradeStock(data = {}, now = new Date()) {
  ...
  // A company with revenue history but no usable free-cash-flow data reports no
  // capital expenditure we can read. Two different situations land here:
  //   - Banks and insurers genuinely have no CapEx in the operating sense, so
  //     free cash flow doesn't apply to them at all.
  //   - REITs, utilities, and smaller filers often DO spend on capital, but tag
  //     it with a company-specific XBRL concept we can't match (e.g. NextEra
  //     files under nee_CapitalExpendituresOfFPL).
  // The message covers both rather than telling a utility it's a bank.
  if (annualRevenues.length >= 2 && annualFreeCashFlows.length < 2) {
    return {
      grade: 'N/A',
      score: 0,
      criteria: [],
      reason: "Free cash flow can't be computed for this company — its filings report no capital expenditure we can read. That's normal for banks and insurers, and also happens with REITs, utilities, and smaller filers that report capital spending under company-specific labels. These are better judged on sector-specific metrics."
    };
  }
  ...
  const criteria = [
    ...
    buildCriterion(
      ...
      earliestFcf,
      'cash flow statement',
      true   // must also be positive — shrinking a loss isn't healthy growth
    ),
    buildCriterion(
